...
router.get("/student/watch-course", authenticate, async(req, res) => {
    try {
        const idUser = req.decoded._id;
        const existedUser = await User.findById(idUser)
            .populate({
            ...
            .lean();
        res.render("profile/student/watch-course", { user: existedUser });
    } catch (err) {
        res.status(400).json({
            message: err,
        });
...
router.delete("/student/wishlist/delete", authenticate, async(req, res) => {
    try {
        ...
        await existedUser.save();
        res.status(200).send({
            message: "Delete this course out of wishlist success",
        });
    } catch (err) {
        res.status(400).json({
            message: err,
